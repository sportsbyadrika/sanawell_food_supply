...
<div class="max-w-7xl mx-auto mt-6 space-y-6 px-4">
    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-5 rounded-xl shadow flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm opacity-90">Search pending bills and capture customer payments.</p>
        </div>
        <a href="index.php?route=bill_list" class="bg-white/20 px-4 py-2 rounded-lg hover:bg-white/30 text-center">Back to Bills</a>
    </div>
    ...
    <div class="grid xl:grid-cols-5 gap-6 items-start">
        <div class="xl:col-span-3 space-y-6">
            <div class="bg-white rounded-xl shadow p-5">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
                    <div>
                        <h2 class="font-semibold text-gray-700">Pending Bills</h2>
                        <p class="text-sm text-gray-500">Search by customer name or mobile number.</p>
                    </div>
                </div>

                <form method="GET" action="index.php" class="grid md:grid-cols-[1fr_auto] gap-3 mb-4">
                    ...
                    >
                    <button class="bg-blue-600 text-white px-5 py-3 rounded-lg hover:bg-blue-700">Search</button>
                </form>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (!empty($pendingBills)): ?>
                                <?php foreach ($pendingBills as $pendingBill): ?>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="p-3">BILL-<?= str_pad((string) $pendingBill['id'], 4, '0', STR_PAD_LEFT) ?></td>
                                        ...
                                        <td class="p-3 text-center">
                                            <?php if ($isPaid): ?>
                                                <button type="button" class="bg-gray-200 text-gray-500 px-3 py-1.5 rounded text-xs cursor-not-allowed" disabled>Pay</button>
                                            <?php else: ?>
                                                <a href="index.php?route=receipt_entry&bill_id=<?= (int) $pendingBill['id'] ?>"
                                                   class="bg-blue-600 text-white px-3 py-1.5 rounded text-xs hover:bg-blue-700">Pay</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="8" class="p-4 text-center text-gray-500">No pending bills found for the current search.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <div class="bg-white rounded-xl shadow p-5">
                ...
                <?php if (empty($bill['id'])): ?>
                    <p class="text-sm text-gray-500">Select a pending bill to view its receipt history.</p>
                <?php else: ?>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                            <thead class="bg-gray-50 text-gray-600 uppercase text-xs">
        ...
        <div class="xl:col-span-2 bg-white rounded-xl shadow p-5 sticky top-4">
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-500">Form is auto-filled from the selected pending bill.</p>
            </div>
            ...
            <?php if (!empty($bill['id'])): ?>
                <div class="mb-4 p-4 rounded-lg bg-blue-50 border border-blue-100 text-sm text-gray-700 space-y-1">
                    ...
                    <p><span class="font-semibold">Customer:</span> <?= htmlspecialchars($bill['customer_name'], ENT_QUOTES, 'UTF-8') ?></p>
            ...
            <form method="POST" action="index.php?route=save_receipt" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="date" name="receipt_date" value="<?= htmlspecialchars((string) ($formDefaults['receipt_date'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" class="w-full border p-2.5 rounded-lg" required>
                </div>
